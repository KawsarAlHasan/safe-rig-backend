...
// submit card by rigadmin
export const submitCardByRigAdminService = async (payload: any) => {
  const {
    companyId,
    rigId,
    resloveRigId,
  ...
  if (isImmediateAction) {
    const rigAdmin = await dbClient.client.findMany({
      where: { optimaiseRigId, companyId },
      select: { name: true, email: true },
    });
    ...
    for (const recipient of recipients) {
      const { html, text } = immediateActionEmailTemplate({
        recipientName: recipient.name || "Unknown",
        submittedBy: employeeName ?? "Unknown",
        isAnonymous: isSubmitAnonymously,
        rigName: optimaiseRigId,
        areaName: String(areaId),
        hazardName: String(hazardId),
        riskSeverity: riskSeverity,
        description: description,
