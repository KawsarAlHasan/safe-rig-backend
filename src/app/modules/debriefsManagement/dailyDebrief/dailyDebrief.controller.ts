...
import {
  checkDebriefService,
  createDebriefByRigAdminService,
...
export const createNewDailyDebriefByRigAdmin = catchAsync(
  async (req: Request, res: Response) => {
    const rigId = resolveRigId(req);
    ...
    const result = await createDebriefByRigAdminService(payload);

    sendResponse(res, {
      success: true,
      statusCode: StatusCodes.OK,
      message: "Daily debrief created successfully completed",
      data: result,
    });
  },
);
