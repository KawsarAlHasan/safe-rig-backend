...
// submit a debrief submit by rig admin
export const createDebriefByRigAdminService = async (payload: any) => {
  const {
    companyId,
    rigId,
    resloveRigId,
  ...
  const isSubmitAnonymously = submitAnonymously == "true" ? true : false;
  const optimaiseRigId = resloveRigId ? resloveRigId : Number(rigId);

  const dailyDebrief = await dbClient.dailyDebrief.create({
    data: {
      companyId: companyId,
      rigId: optimaiseRigId,
...
export const getAllActiveDebriefService = async (
  ...
  const debriefQuestion = await dbClient.debriefQuestion.findMany({
    where: {
      companyId: companyId,
      status: "ACTIVE",
      OR: [
  ...
  return { activity, typeOfDevrief, debriefQuestion };
};
